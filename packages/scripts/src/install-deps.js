...
async function getAppName() {
  const pkg = JSON.parse(
    await utils.promisify(fs.readFile)(
      path.resolve(__dirname, paths.electron, 'package.json'),
      'utf-8'
    )
  )
...
async function getPathPkgMonoRepo(names) {
  const pathPkgs = await utils.promisify(glob)(
...
module.exports = async function installDeps() {
  ...
  try {
    await utils.promisify(fs.stat)(PACKAGING_PATH)

    const appName = await getAppName()
    const pkgs = await utils.promisify(glob)(
      path.join(PACKAGING_PATH, `${appName}*/**/package.json`)
    )
    const pkg = pkgs[0]
